            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {COLOR_OPTIONS.map(c => (
                    <button key={c} onClick={() => setForm(f => ({ ...f, color: c }))} className={`h-7 w-7 rounded-full border-2 transition-transform ${form.color === c ? "border-foreground scale-110" : "border-transparent"}`} style={{ backgroundColor: c }} />
                  ))}
                </div>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <MoneyInput value={form.price} onChange={(v) => setForm(f => ({ ...f, price: Number(v) }))} placeholder="0,00" />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Comissão (%)</Label>
                <Input type="number" step="0.01" min="0" max="100" value={form.commission_percent} onChange={e => setForm(f => ({ ...f, commission_percent: Number(e.target.value) }))} />
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              <Button size="sm" onClick={() => handleSave()}><Save className="h-3.5 w-3.5 mr-1" /> Salvar</Button>
              <Button size="sm" variant="ghost" onClick={() => { setShowNew(false); setForm({ name: "", color: "#3B82F6", duration_minutes: 60, price: 0, commission_percent: 0 }) }}><X className="h-3.5 w-3.5 mr-1" /> Cancelar</Button>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <EmptyState
              ...
              description="Adicione seu primeiro serviço para começar a agendar."
            />
          ) : (
            <div className="divide-y divide-border/40">
              {services.map((svc) => (
                <div key={svc.id} className={`flex flex-col gap-3 p-4 hover:bg-muted/50 transition-colors sm:flex-row sm:items-center sm:gap-4 ${!svc.active ? "opacity-50" : ""}`}>
                  {editing === svc.id ? (
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                      ...
                      <div className="flex gap-1">
                        <Button size="sm" onClick={() => handleSave(svc.id)}><Save className="h-3 w-3" /></Button>
                    ...
                    <>
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="h-10 w-10 shrink-0 rounded-xl flex items-center justify-center" style={{ backgroundColor: svc.color + "20" }}>
                          <div className="h-3.5 w-3.5 rounded-full" style={{ backgroundColor: svc.color }} />
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-sm truncate">{svc.name}</p>
                          <p className="text-xs text-muted-foreground tnum">{svc.duration_minutes}min • R$ {svc.price.toFixed(0)}{svc.commission_percent > 0 ? ` • ${svc.commission_percent}% comissão` : ""}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-0.5 sm:shrink-0">
                        <Switch
                          size="sm"
                          checked={svc.active}
                          onCheckedChange={() => handleToggleActive(svc.id, svc.active)}
                          aria-label={`Ativar ou desativar ${svc.name}`}
                        />
                        <Button size="icon" variant="ghost" className="h-9 w-9 rounded-lg" title="Kit de materiais" onClick={() => openKit(svc)}>
                          <Package className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-9 w-9 rounded-lg" title="Editar" onClick={() => { setEditing(svc.id); setForm({ name: svc.name, color: svc.color, duration_minutes: svc.duration_minutes, price: svc.price, commission_percent: svc.commission_percent }) }}>
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-9 w-9 rounded-lg text-destructive" title="Excluir" onClick={() => handleDelete(svc.id)}>
