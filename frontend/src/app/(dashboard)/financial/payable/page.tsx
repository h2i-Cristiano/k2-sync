...
export default function PayablePage() {
  ...
  const handleSave = async (id?: string) => {
    ...
    if (id) {
      ...
      const { error } = await supabase.from("financial_entries").insert({ ...form, notes: form.notes || null, type: "payable", tenant_id: tenantId })
      ...
      <Card className="glass-card">
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <p className="text-center py-8 text-muted-foreground text-sm">Nenhuma conta pendente.</p>
          ) : (
                  ...
                  {editing === entry.id ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 rounded-lg" onClick={() => { setEditing(entry.id); setForm({ description: entry.description, amount: entry.amount, due_date: entry.due_date, category: entry.category || "Outros", notes: entry.notes || "" }) }}><Pencil className="h-3.5 w-3.5" /></Button>
      ...
      <Card className="glass-card">
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <p className="text-center py-8 text-muted-foreground text-sm">Nenhuma conta paga.</p>
          ) : (
            <div className="divide-y divide-border/40">
